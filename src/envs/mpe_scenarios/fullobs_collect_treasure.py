...
class Scenario(BaseScenario):
    def make_world(self):
        world = World()
        # add the episode_limit property for pymarl
        world.episode_limit = 100
    ...
    def observation(self, agent, world):
        ...
        treasure_holding_flag = np.arange(n_treasure_types) == agent.holding
        if agent.collector:
            # collectors need to know their own state bc it changes
            obs.append((treasure_holding_flag))
        else:
            obs.append((np.zeros_like(treasure_holding_flag)))
